...
class windowStack {
	...
	hadFocus: boolean;
	hookedMouse: boolean;
	events: (string | ((event: any) => void))[][];
	leftPos: number;
	topPos: number;
	mousedInsideWindow: boolean;
	movable: boolean;
	leftLast: any;
	topLast: any;
	resize: boolean;
	coordDiffLeft: number;
	coordDiffTop: number;
	lastLeft: number;
	lastTop: number;
	constructor(element) {
	...
	getStyleFloatOf(propertyNameOf, JSpropertyNameOf) {
		try {
			var dirtyValue = (window.getComputedStyle(this.domObject, null).getPropertyValue(propertyNameOf) as any);
		}
		catch (error) {
			try {
